// Clear rows in the Intensitas Bunyi sheet for the authenticated Clerk user.
const corsHeaders = {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const readUrl = `${baseUrl}/values/${SHEET_TAB_ENC}!A7:H?valueRenderOption=UNFORMATTED_VALUE`;
    ...
    const ranges = requested.map((n) => `${SHEET_TAB}!A${n}:H${n}`);
